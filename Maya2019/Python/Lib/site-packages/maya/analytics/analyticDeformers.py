...
kAnalyticLabel = maya.stringTable['y_analyticDeformers.kAnalyticLabel' ]
kAnalyticDescriptionShort = maya.stringTable['y_analyticDeformers.kAnalyticDescriptionShort' ]
...
class analyticDeformers(BaseAnalytic):
    ...
    def run(self):
        ...
        try:
            if len(meshList) == 0:
                return { 'status' : 'No meshes to check' }
        except Exception, ex:
            # If the 'ls' command returns None this is the easiest
            # way to trap that case.
            return { 'status' : 'Mesh check failed ({0:s})'.format(str(ex)) }
        ...
        try:
            ...
            return { 'status' : 'Deformer check failed ({0:s})'.format(str(ex)) }
