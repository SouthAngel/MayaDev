...
class Runner(object):
    ...
    def __init__(self):
        ...
        TODO( "FEATURE", "Add a version number so that analytics can be out of date by virtue of a new version number as well", None )
    ...
    def __files_to_analyze(self):
        ...
        for path in self.paths:
            ...
            dir_list = get_maya_files( iterator )

            files_to_analyze = []
            for (the_dir, the_files) in dir_list:
    ...
    def run(self):
                ...
                try:
                    ...
                    if not file_is_loaded:
                        try:
                            ...
                            memory_before = cmds.memory( heapMemory=True )
                            ...
                            start_time = cmds.timerX()
                            cmds.refresh()
                            first_refresh_time = cmds.timerX( st=start_time )
                            memory_delta = memory_before - cmds.memory( heapMemory=True )
                        except Exception, ex:
                    ...
                    if file_load_time > 0.0:
                        ...
                        json_data['first_refresh_time'] = first_refresh_time
                        json_data['memory_delta'] = memory_delta
                    ...
                    if self.return_json:
                        json_per_file.update( json_data )
        ...
        if len(combinations_run) > 0:
            ...
            self.__warning( 'No analytics run with these settings:\n{}'.format( str(self) ) )
