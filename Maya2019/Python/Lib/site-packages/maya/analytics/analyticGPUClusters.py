"""
Analyze the usage modes of cluster nodes.
"""
import maya
...
kAnalyticLabel = maya.stringTable['y_analyticGPUClusters.kAnalyticLabel' ]
kAnalyticDescriptionShort = maya.stringTable['y_analyticGPUClusters.kAnalyticDescriptionShort' ]
    ...
    def run(self):
        ...
        with emModeManager() as em_manager:
            ...
            try:
                ...
                return { 'error' : 'Graph examination failure ({0:s})'.format( str(ex) ) }

        if not animated_nodes:
            return { 'warning' : 'No GPU animation to examine' }
        ...
        if not cluster_nodes:
            return { 'warning' : 'No GPU clusters to examine' }
