...
def switchMayaViewport(newMode):
    ...
    if newMode == emPerformanceOptions.VIEWPORT_1:
        ...
        raise NameError( maya.stringTable['y_emPerformanceTest.kSwitchViewportError' ] % str(newMode) )
...
class emPerformanceRun(object):
    ...
    def testTheType(self, theTestType):
        ...
        if theTestType == emPerformanceOptions.TEST_PLAYBACK:
            ...
            raise NameError( maya.stringTable['y_emPerformanceTest.kTestTheTypeError' ] % str(theTestType) )
    ...
    def runTests(self):
        ...
        with CacheEvaluatorManager() as cacheMgr:
            cacheMgr.enabled = False
            with emModeManager() as mgr:
                ...
                if self.fileName:
                    ...
                    self.progress.startPhase( maya.stringTable['y_emPerformanceTest.kReadingMessage' ] )
                        ...
                        for testType in self.options.testTypes:
                            ...
                            if not timingResults:
                                ...
                                if len(timingResults) == 2:
                                    ...
                                    raise TypeError( maya.stringTable['y_emPerformanceTest.kDirtyRefreshError' ] % str(timingResults) )
                ...
                if self.fileName:
                    self.progress.startPhase( maya.stringTable['y_emPerformanceTest.kNewMessage' ] )
    ...
    def testPlayback(self):
        ...
        try:
            with PlaybackManager() as play_mgr:
                ...
                # Keep the frames played to a minimum - enough to start the motion. Anything else
                # is wasted since evaluation will almost always be the same.
                (self.startFrame, self.endFrame) = play_mgr.set_limited_range( EMPERFORMANCE_PLAYBACK_MAX, from_start=True )
                ...
                self.progress.startPhase( maya.stringTable['y_emPerformanceTest.kPlayMessage' ] % (self.evalMode, self.options.iterationCount) )
    ...
    def testRefresh(self):
        ...
        self.progress.startPhase( maya.stringTable['y_emPerformanceTest.kRefreshMessage' ] % (self.evalMode, self.options.iterationCount * 10) )
    ...
    def testDirtyRefresh(self):
        ...
        # Since refreshes are fast compared to playback do it 10 times
        self.progress.startPhase( maya.stringTable['y_emPerformanceTest.kDirtyRefreshMessage' ] % (self.evalMode, self.options.iterationCount * 10) )
        for idx in range(0, self.options.iterationCount * 10):
...
class emPerformanceOptions(object):
    ...
    # Default column titles and stats
    # These fields are used by the toolkit, in order to get the correct localization.
    COLUMN_TITLE_START_FRAME = maya.stringTable['y_emPerformanceTest.kColumnTitleStartFrame']
    COLUMN_TITLE_END_FRAME = maya.stringTable['y_emPerformanceTest.kColumnTitleEndFrame']
    COLUMN_STAT_AVG = maya.stringTable['y_emPerformanceTest.kColumnTitleAvg' ]
    COLUMN_STAT_MAX = maya.stringTable['y_emPerformanceTest.kColumnTitleMax' ]
    COLUMN_STAT_MIN = maya.stringTable['y_emPerformanceTest.kColumnTitleMin' ]
    COLUMN_STAT_TIME = maya.stringTable['y_emPerformanceTest.kColumnTitleTime']
    ...
    def __init__(self):
        ...
        self.testTypes = [ emPerformanceOptions.TEST_PLAYBACK
    ...
    def setEvalModes(self, newEvalModes):
        ...
        """
        if isinstance(newEvalModes, list):
            self.evalModes = [string.lower(mode) for mode in newEvalModes]
        else:
            self.evalModes = [string.lower(newEvalModes)]
    ...
    def setTestTypes(self, newTestTypes):
        ...
        You can pass in a single value or a list of values.
        """
        if isinstance(newTestTypes, list):
    ...
    def setViewports(self, newViewports):
        ...
        You can pass in a single value or a list of values.
        """
        if isinstance(newViewports, list):
    ...
    def columnTitles(self):
        ...
        """
        if self.allColumns:
            stats = [ self.COLUMN_STAT_AVG
                    , self.COLUMN_STAT_MIN
                    , self.COLUMN_STAT_MAX ]
        else:
            stats = [self.COLUMN_STAT_TIME]
        columns = []
        extras = []
        if emPerformanceOptions.TEST_PLAYBACK in self.testTypes:
            if self.allColumns:
                columns += [ self.COLUMN_TITLE_START_FRAME
                           , self.COLUMN_TITLE_END_FRAME ]
            # For playback in EM there is also collection of graph creation and
            # scheduling time. Add those to the result tyeps to collect.
            ...
            for mode in self.evalModes:
                for resultType in self.testTypes + extras:
                    ...
                    if (mode == emPerformanceOptions.EVALUATION_MODE_DG) and (resultType in extras):
                        continue
                    for stat in stats:
                        columns += [self.getColumnTitle(viewport, resultType, mode, stat)]
        return columns
    ...
    def getColumnTitle(self, viewport, type, mode, stat):
        ...
        """
        return '%s %s %s %s' % (viewport, type, string.upper(mode), stat)
...
class progressState(object):
    ...
    def __init__(self, fileCount, phaseCount, enable):
        ...
        self.enable = enable
        self.currentFileName = maya.stringTable['y_emPerformanceTest.kStartup' ]
        self.progressPerFile = 1.0 / fileCount
        self.totalProgress = 0
        self.currentFile = 0
        self.currentPhase = 0
        self.currentPhaseName = maya.stringTable['y_emPerformanceTest.kReadingInitPhase' ]
        self.progressPerPhase = self.progressPerFile / phaseCount
        if enable:
            winTitle = maya.stringTable['y_emPerformanceTest.kProcessingMessage' ] % fileCount
            cmds.progressWindow( title=winTitle, progress=0, status=maya.stringTable['y_emPerformanceTest.kInitializingMessage' ], isInterruptable=True )
    ...
    def startFile(self, newFileName):
        ...
        self.currentFile += 1
        self.currentPhase = 0
        self.currentPhaseName = maya.stringTable['y_emPerformanceTest.kReadingPhase' ]
...
def emPerformanceTest(
    ...
    if filesAndDirectories:
        for path in filesAndDirectories:
            if os.path.isdir( path ):
                ...
                print maya.stringTable['y_emPerformanceTest.kWrongFileWarning' ] % path

        if len(mayaFiles) == 0:
            print maya.stringTable['y_emPerformanceTest.kNoFileError' ]
            return
    else:
        mayaFiles = [None]

    kExceptionError = maya.stringTable['y_emPerformanceTest.kExceptionError' ]
    ...
    if options.usesEvaluationManager():
        if not evaluationManagerExists():
            print maya.stringTable['y_emPerformanceTest.kNoEvaluationManagerError' ]
    ...
    for fileName in mayaFiles:
        if fileName:
            ...
            currentProgress.startFile( maya.stringTable['y_emPerformanceTest.kCurrentScene' ] )
