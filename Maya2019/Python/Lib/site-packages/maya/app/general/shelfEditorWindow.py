...
class shelfEditorWindow:
  ...
  def __init__(self, windowName="shelfEditor"):
    ...
    self.mainShelfLayout = mel.eval('global string $gShelfTopLevel; format -stringArg $gShelfTopLevel "^1s"')
  ...
  # ---------------------------------------------------------------------------
  def flexibleWidthChange(self, data):
    # Make sure the image is reloaded
    currItem = self.currentItem()
    path = cmds.shelfButton(currItem, query=True, image=True)
    ...
    if path is not None:
      ...
      if cmds.toolButton(currItem, exists=True ):
        ...
        newFlexWidth = cmds.optionMenuGrp(self.wIconWidthOptionMenu, query=True, select=True)

        if newFlexWidth == 2:
          # If it's a new change to the Custom width, wait for the user to enter the width
          if cmds.textFieldGrp(self.wIconWidthValue, query=True, enable=True) == False:
            cmds.textFieldGrp(self.wIconWidthValue, edit=True, enable=True)
          
          wantedWidthValue = int(cmds.textFieldGrp(self.wIconWidthValue, query=True, text=True))
          if wantedWidthValue > 0:
            # Update the icon
            cmds.shelfButton(currItem, edit=True, image=path,
                flexibleWidthType=newFlexWidth, 
                flexibleWidthValue=wantedWidthValue)
        else:
            # Update the icon
          cmds.textFieldGrp(self.wIconWidthValue, edit=True, enable=False)
          cmds.shelfButton(currItem, edit=True, image=path,
            flexibleWidthType=newFlexWidth)
          # Update the text field with the pixmap size set.
          cmds.textFieldGrp(
            self.wIconWidthValue, edit=True, 
            text=(cmds.shelfButton(currItem, query=True,flexibleWidthValue=True)))
  # ---------------------------------------------------------------------------
  ...
  def updateItemData(self):
    ...
    if (currItem is None) or cmds.separator(currItem, exists=True):
      ...
      cmds.checkBoxGrp(self.wItemRepeatable, edit=True, enable=False,
                       value1=False)
      cmds.optionMenuGrp(self.wIconWidthOptionMenu, edit=True, enable=False)
      cmds.textFieldGrp(self.wIconWidthValue, edit=True, enable=False, text='')
    ...
    cmds.optionMenuGrp(self.wIconWidthOptionMenu, edit=True, enable=True,
      select=cmds.shelfButton(currItem, query=True, flexibleWidthType=True))

    isCustom = cmds.shelfButton(currItem, query=True, flexibleWidthType=True) == 2
    widthValue = cmds.shelfButton(currItem, query=True, flexibleWidthValue=True)
    cmds.textFieldGrp(self.wIconWidthValue, edit=True, enable=isCustom, text=widthValue)
  ...
  def createShelfData(self):
    ...
    self.wIconWidthOptionMenu = cmds.optionMenuGrp(
        label=maya.stringTable['y_shelfEditorWindow.kShelfIconWidth' ],
        adjustableColumn=2,
        columnAttach=[(1, "right", 0), (2, "left", 0)],
        columnWidth=[(1, self.col1width), (2, 75)],
        changeCommand=self.flexibleWidthChange)

    cmds.menuItem( label=maya.stringTable['y_shelfEditorWindow.kIconWidthStandard'  ] )
    cmds.menuItem( label=maya.stringTable['y_shelfEditorWindow.kIconWidthCustom'    ] )
    cmds.menuItem( label=maya.stringTable['y_shelfEditorWindow.kIconWidthAutomatic' ] )

    self.wIconWidthValue = cmds.textFieldGrp(
      label="",
      adjustableColumn=2,
      columnWidth=(1, self.col1width),
