...
# Import available PySide2 or PyQt package, as it will work with both
try:
...
logger.setLevel(logging.WARNING)  # Forcing logger to warnings/errors explicitly until MAYA-67049 fixed
...
class CreaseSetTreeWidget(QTreeWidget):
    ...
    def setItemValues(self, item=None, name=None, colorIndex=None, creaseLevel=None, members=None, numSelectedMembers=None, showSetMembers=False):
        ...
        '''
        if isinstance(item, types.NoneType):  # NOTE: Using isinstance() instead of item==None since PySide2 does not allow for that __eq__ operator
