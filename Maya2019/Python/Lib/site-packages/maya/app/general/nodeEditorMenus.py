...
def _createShaderMenuItems(ned, node):
    ...
    # The "Assign Material Override" option is not supported by Render Setup, only by Legacy Render Layers.
...
def createConnectionMarkingMenu(ned):
    ...
    def setWatchpointCB(*args):
        nEd(ned, e=True, setWatchpoint=True)

    hasWatchpoint = nEd(ned, q=True, hasWatchpoint=True)
    ...
    if inContainer:
        cmds.menuItem(label= maya.stringTable['y_nodeEditorMenus.kAddWatchpoint' ], radialPosition = 'S',
        ann= maya.stringTable['y_nodeEditorMenus.kAddWatchpointAnnot' ],
        c = setWatchpointCB, enable = (not hasWatchpoint))
