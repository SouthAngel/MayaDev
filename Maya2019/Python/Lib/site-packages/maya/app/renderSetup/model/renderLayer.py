...
import maya.app.renderSetup.model.renderSetupPreferences as renderSetupPreferences
...
kAOVCollectionNodeCreationFailed = maya.stringTable['y_renderLayer.kAOVCollectionNodeCreationFailed' ]
...
kCreateLightsChildCollection   = maya.stringTable['y_renderLayer.kCreateLightsChildCollection'    ]
...
class RenderLayerBase(object):
    ...
    def __init__(self):
        ...
        # we use a cache for the legacy node name because during an import/export operation,
        # there is a point where we need to access the legacy node name but access is restricted.
        self.legacyNodeNameCache = None
...
class DefaultRenderLayer(RenderLayerBase, Observable):
    ...
    def getLegacyNode(self):
        ...
        return commonUtils.nameToNode(self._getLegacyNodeName())
...
class RenderLayer(RenderLayerBase, nodeList.ListBase, childNode.ChildNode):
    ...
    def __init__(self):
        ...
        # Initially, there is no member cache.  Note that this is
        # logically different from an empty cache; see MAYA-81869.
        self._memberCache = None
        self.isUpdatingMembership = False
        self.needsMembershipUpdate = False
        self._objectDeleted = False
    ...
    def _getLegacyNodeName(self):
        out = utils.getSrcNodeName(utils.findPlug(self, RenderLayer.legacyRenderLayer))
        # During Maya file import and export, accessing attribute RenderLayer.legacyRenderLayer fails. Use a cached version in that case.
        if out is not None:
        # If the legacy render layer full name is returned with a prepended ':', remove it.
            self.legacyNodeNameCache = out.lstrip(':')
        return self.legacyNodeNameCache

    def getLegacyNode(self):
        '''Return the legacy render layer MObject.'''
        return utils.getSrcNode(utils.findPlug(self, RenderLayer.legacyRenderLayer))
    ...
    def setMemberNodesCache(self, cache):
        assert isinstance(cache, set), 'Render layer member cache must be a set.'
        self._memberCache = cache

    def getMemberNodesCache(self):
        return self._memberCache
    ...
    def _updateLegacyRenderLayerVisibility(self):
        ...
        try:
            with profiler.ProfilerMgr('RenderLayer::_updateLegacyRenderLayerVisibility'):
                ...
                if currentlySelectedNodeNames!=self.getMemberNodesCache():
    ...
    def addMembers(self, objs):
        """ Add the list of objects to the legacy layer members if the current
            layer is the visible one."""
        ...
        if self.isVisible():
            ...
            if self.getMemberNodesCache() is None:
                self.setMemberNodesCache(set())
            self.getMemberNodesCache().update(path.fullPathName() for path in paths)
        return paths

    def addDefaultMembers(self, objs):
        paths = self.addMembers(objs)
        self.getDefaultCollection().getSelector().staticSelection.add(paths)
    
    def _startMembershipUpdate(self):
        # If we're in batch mode, evalDeferred reduces to eval, i.e. immediate,
        # synchronous execution.  This is a problem if we're being called
        # (indirectly) from a scene changed callback.  For example, node
        # removal from the graph sends multiple notifications.  In immediately
        # updating membership on first notification, we could add a node that
        # is in the process of being removed, which causes crashes.
        #
        # Trying to reproduce the behavior of evalDeferred using Python's
        # non-blocking threading.Timer or sched.scheduler fails, as Timer code
        # runs in another thread, which Maya commands cannot (and assert), and
        # sched.scheduler is blocking.
        #
        # Asynchronous membership update is thus an interactive convenience,
        # and does not change the fact that membership may need updating after
        # a scene graph change.
        if cmds.about(batch=True) and sceneObservable.isInSceneChangeCallback():
    ...
    @_visible
    def _collectionChanged(self,collection):
    ...
    def _selectedNodesChanged(self, collection):
        ...
        if self.needsMembershipUpdate:
            self.clearMemberNodesCache()

        # If we're not visible, no need to do an apply update.  Similarly, the
        # membership update will be done later, when we become visible.
        if not self.isVisible():
            return

        if self.needsMembershipUpdate:
            self._startMembershipUpdate()
        # set needsApplyUpdate conservatively. 2 possible cases:
        # - if the collection has any override in itself or in its child
        #   collections and the selection changed, then there might be objects
        #   in it that are not overridden by the overrides in that
        #   collection/subcollections, or objects that are still overridden
        #   but should no longer be.
        # - if the collection is traversing connections to populate itself,
        #   then reevaluating its content may give wrong results as subsequent
        #   connection override may be applied and the scene is thus not in the
        #   state it would have been when sequentially applying
        #   collections/overrides on layer
        # However, we do not need to apply an update if the collection or any
        # of its parents is empty.
        self.needsApplyUpdate = self.needsApplyUpdate or \
            (not collection.isEmptyRecursive() and
             (collection.isTraversingConnections() or collection.hasOverridesRecursive()))
    ...
    def getRenderSettingsChildCollectionByName(self, renderSettingsChildCollectionName, nested=False):
        ...
        if not renderSettingsChildCollectionName:
            raise Exception(kInvalidCollectionName)

        for child in nodeList.forwardListGenerator(self.renderSettingsCollectionInstance()):
            if child.name() == renderSettingsChildCollectionName:
                return child
            elif nested and child.typeId() == typeIDs.renderSettingsChildCollection:
                child2 = child.getRenderSettingsChildCollectionByName(renderSettingsChildCollectionName, True)
                if child2:
                    return child2

        raise Exception(kUnknownCollection % (renderSettingsChildCollectionName, self.name()))
    ...
    def hasApplyOverridesRecursive(self):
        for ov in utils.getOverridesRecursive(self):
            if ov.hasApplyOverrides():
                return True
        return False
    ...
    def aovCollectionInstance(self):
        ...
        if aovCollection is None:
            OpenMaya.MGlobal.displayError(kAOVCollectionNodeCreationFailed)
    ...
    def getMembers(self):
        ...
        with profiler.ProfilerMgr('RenderLayer::getMembers'):
            ...
            lightsAddedByDefaultToLayer = renderSetupPreferences.IncludeAllLightsSetting.isEnabled()
            
            if not self.hasLightsCollectionInstance() and lightsAddedByDefaultToLayer:
            ...
            for c in traverse.depthFirst(self, memberTraversal):
                ...
                if not lightsAddedByDefaultToLayer and isinstance(c, collection.LightsCollection):
                    continue
    ...
    def getCorrespondingCollection(self, nodeName, selectedCollectionName):
        ...
        if collection.RenderSettingsCollection.containsNodeName(nodeName):
            return self.getRenderSettingsChildCollectionByName(selectedCollectionName, True) if selectedCollectionName else self.renderSettingsCollectionInstance()
        ...
        try:
            ...
            coll = None
        # Search if the node is part of the Lights Collection
        if not coll and collection.LightsCollection.containsNodeName(nodeName):
            lightsCol = self.lightsCollectionInstance()
            # Look for a child collection whose selection contains the node
            for childColl in lightsCol.getCollections():
                if childColl and nodeName in list(childColl.getSelector().names()):
                    return childColl

            # If it doesn't exist, create a new child node
            with undo.CtxMgr(kCreateLightsChildCollection % self.name()):
                # Check if another node should be used as stand-in
                other = lightsCol.getSelector().getSelectionStandIn(nodeName)
                if other:
                    nodeName = other

                # Create a new Lights child collection and put it last in the list
                coll = collection.create(nodeName+"_col", collection.LightsChildCollection.kTypeId)
                coll.getSelector().setStaticSelection(nodeName)
                lightsCol.appendChild(coll)
                return coll
    ...
    @undo.chunk('Create a connection override')
    def createConnectionOverride(self, nodeName, attrName, collectionName=None):
        """ Add a connection override to a collection """
        return self.getCorrespondingCollection(nodeName, collectionName).createConnectionOverride(nodeName, attrName)
    ...
    @context.updateLayer
    def update(self):
        with profiler.ProfilerMgr('RenderLayer::update'):
            for col in nodeList.forwardListGenerator(self):
                col.update()
    ...
    def _encodeProperties(self, dict):
        ...
        dict[jsonTranslatorGlobals.RENDERABLE_ATTRIBUTE_NAME] = self.isRenderable()
    ...
    def copyForClipboard(self):
        data = self.encode()
        data[self.typeName()][
            jsonTranslatorGlobals.VISIBILITY_ATTRIBUTE_NAME] = False
        return data
    ...
    def _decodeProperties(self, dict, mergeType, prependToName):
        ...
        self.setRenderable(dict.get(jsonTranslatorGlobals.RENDERABLE_ATTRIBUTE_NAME, True))
