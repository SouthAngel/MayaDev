...
def isVisible(legacyRenderLayerName):
    with profiler.ProfilerMgr('legacyRenderLayer::isVisible'):
        # lstrip added because during an import/export operation, editRenderLayerGlobals returns the full current name including its namespace and causing an undesired mismatch.
...
def isRenderable(legacyRenderLayerName):
    with profiler.ProfilerMgr('legacyRenderLayer::isRenderable'):
        # colon added  because during an import/export operation, the full name of the legacy render layer is looked for.
