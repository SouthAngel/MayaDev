...
# List of render setup and legacy render layers node types with the exclude
# symbol in front. This list is used to add Render Setup and legacy render
# layer nodes to the list of excluded node types.
rsExcludes = []
...
def getRSExcludes():
    ...
    global rsExcludes
    if not rsExcludes:
        rsExcludes = list(itertools.imap(lambda x: '-' + x, utils.getRenderSetupNodeTypesNames() + utils.getLegacyRenderLayerNodeTypesNames()))
    return rsExcludes
...
def findSetMembersRecursively(members):
    ...
    for member in members:
        path = commonUtils.nameToDagPath(member)
        if path is not None:
            yield path
        else:
            # Note: It is sufficient to return the dagNodes in the set (using
            # the flag nodesOnly) instead of all dag paths (there could be many
            # for one given node), because legacy render layer membership does
            # not take dagPaths into account. Even if it is given dagPaths, it
            # will add the corresponding nodes to its membership.
            children = cmds.sets(member, nodesOnly=True, q=True) or []
            for childDagPath in findSetMembersRecursively(children):
                yield childDagPath
...
def ls(patterns, types=None):
    ...
    # Render Setup and Legacy Render Layer nodes should never be added as
    # members of a selector.
    # WARNING: Iterating through the Render Setup node type names for each
    # node in the selection whenever this ls is called (which happens often
    # when the corresponding render layer is visible) might lead to significant
    # performance loss. Therefore, if it is deemed too performance heavy, the
    # following line will have to be removed and another solution found.
    excludeTypes.extend(getRSExcludes())
...
class Selector(OpenMaya.MPxNode):
    ...
    def activate(self):
        ...
        observable = sceneObservable.instance()
        for cbtype, callback in self._bulkChangeCallbacks.iteritems():
            observable.register(cbtype, callback)
...
class DagStrategy(Strategy):
    ...
    def items(self, selection):
        ...
        setmembers = (name for name in findSetMembersRecursively(sets))
        return itertools.ifilter(self.filterFunct, itertools.chain(selection.shapes() if self.shapesOnly else selection.hierarchy(),setmembers))
...
class ShadingEngineStrategy(TraversingConnectionStrategy):
    ...
    def onConnectionChanged(self, selector, srcPlug, dstPlug, made):
        if not selector.parent():
            return # only traversing connection from parent's content
        # shape => shadingEngine
...
class SetStrategy(TraversingConnectionStrategy):
        ...
        def _findSetsRecursive(nodes):
            ...
            for node in nodes:
                if node.hasFn(OpenMaya.MFn.kSet):
                    ...
                    members = (selListMembers.getDependNode(i) for i in xrange(0, selListMembers.length()))
    ...
    def onConnectionChanged(self, selector, srcPlug, dstPlug, made):
        # dag => set or set => set
        if dstPlug.node().hasFn(OpenMaya.MFn.kSet):
    ...
    def members(self, selector):
        ...
        return (member for member in findSetMembersRecursively(cmds.sets(selector.names(), nodesOnly=True, q=True) or []))
...
class SimpleSelector(Selector):
    ...
    def strategy(self, dataBlock=None):
        if dataBlock or not self._strategy:
            ...
            strategies = filter(None, (cls.create(filterType, customs) for cls in [DagStrategy, ShadingStrategy, ShadingEngineStrategy, GeneratorStrategy, SetStrategy, NonDagStrategy]))
...
class LightsCollectionSelector(SimpleSelector):
    ...
    def _update(self, dataBlock):
        ...
        # Add in any light items attached to light sources.
        self._cache |= set(LightsCollectionSelector._findLightItems(self._cache))
