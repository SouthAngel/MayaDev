...
kWrongArgTypeCtxMgr = maya.stringTable['y_utils.kWrongArgTypeCtxMgr'  ]
...
def deleteNode(node):
    ...
    cmds.delete(node if isinstance(node, basestring) else node.name())
...
class EncodeMasterValuesCtxMgr(object):
    """ Unapply all overrides from the list of nodes that depends on the
        ...
        master layer.
    """
    # NOTE: This context manager is currently only compatible
    #       with classes RenderSettings and AOVs.

    def __init__(self, getAllNodesFunction):
        self._overridesCache = list()
        self.getAllNodesFunction = getAllNodesFunction

    def unapplyCorrespondingOverrideAndAddToCache(self, applyOvr):
        currOvr = applyOvr.override()
        self._overridesCache.append(currOvr)
        currOvr.unapply()
    ...
    def __enter__(self):
        ...
        # If the master layer is visible, the master values will get encoded.
        if not renderSetupModel.hasInstance() or renderSetupModel.instance().getDefaultRenderLayer().isVisible():
            return
        # Iterate the nodes from the given function and unapply all
        # overrides corresponding to their chains of apply overrides
        for node in self.getAllNodesFunction():
            dependNode = OpenMaya.MFnDependencyNode(node)
            nodeConnections = dependNode.getConnections()
            for nodeCon in nodeConnections:
                src = getSrcUserNode(nodeCon)
                # We get and unapply all apply overrides in the apply chain
                while isinstance(src, applyOverride.ApplyOverride):
                    ovrToBeUnapplied = src
                    src = getSrcUserNode(ovrToBeUnapplied.getOriginalPlug())
                    self.unapplyCorrespondingOverrideAndAddToCache(ovrToBeUnapplied)

                destinations = getDstUserNodes(nodeCon) or []
                # Iterator yielding all Apply Override destinations from node
                dst = next((d for d in destinations if isinstance(d, applyOverride.ApplyOverride)), None)
                while isinstance(dst, applyOverride.ApplyOverride):
                    ovrToBeUnapplied = dst
                    dst = ovrToBeUnapplied.prev()
                    self.unapplyCorrespondingOverrideAndAddToCache(ovrToBeUnapplied)

    def __exit__(self, type, value, traceback):
        for ovr in self._overridesCache:
            ovr.postApply()

def encodeMasterValues(getAllNodesFunction):
    def decorator(f):
        def wrapper(*args, **kwargs):
            with EncodeMasterValuesCtxMgr(getAllNodesFunction):
                return f(*args, **kwargs)
        return wrapper
    return decorator
...
def sceneHasApplyOverridesOnAGivenNodeType(getAllNodesFunction):
    ...
    import maya.app.renderSetup.model.renderSetup as renderSetupModel
    import maya.app.renderSetup.model.applyOverride as applyOverride
    # If the Render Setup singleton node was not created yet or the master
    # layer is visible, an assumption can be made that there are no apply nodes.
    if not renderSetupModel.hasInstance() or renderSetupModel.instance().getDefaultRenderLayer().isVisible():
        return False
    # Iterate the nodes from the given function and try to find any apply
    # overrides on them, as source or destination.
    for node in getAllNodesFunction():
        dependNode = OpenMaya.MFnDependencyNode(node)
        nodeConnections = dependNode.getConnections()
        for nodeCon in nodeConnections:
            src = getSrcUserNode(nodeCon)
            if isinstance(src, applyOverride.ApplyOverride):
                return True
            destinations = getDstUserNodes(nodeCon) or []
            for d in destinations:
                if isinstance(d, applyOverride.ApplyOverride):
                    return True
    return False

def getRenderSetupNodeTypes():
    """ Returns a tuple of concrete/usable Render Setup node types """
    from maya.app.renderSetup.model.renderSetup import RenderSetup
    ...
    return (RenderSetup, RenderLayer, Collection, Selector, Override, ApplyOverride)

def getRenderSetupNodeTypesNames():
    return [node.kTypeName for node in getRenderSetupNodeTypes()]

def getLegacyRenderLayerNodeTypesNames():
    return ['renderLayer', 'renderLayerManager']
