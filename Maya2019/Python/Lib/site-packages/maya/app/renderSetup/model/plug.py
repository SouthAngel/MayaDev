...
class Plug(object):
    ...
    def _isVector(type):
        ...
        return isinstance(type,list) and len(type) <= 3 and len(type) > 0 and not isinstance(type[0],list) and (len(set(type))==1) and type[0] in Plug._simpleNumericTypes
    ...
    _accessors = {
        ...
        kFilename : lambda plug: plug._getPlugValue(OpenMaya.MPlug.asString),
    ...
    _mutators = {
        ...
        kFilename : lambda plug, value: plug._setPlugValue(OpenMaya.MDGModifier.newPlugValueString, value),
    ...
    _creators = {
        ...
        kFilename : (OpenMaya.MFnTypedAttribute,   _createFilenameAttr),
    ...
    def _creator(type):
        if isinstance(type, list):
            ...
            def creator(longName, shortName):
                children = [creator(longName+str(i), shortName+str(i)) for (i, (_, creator)) in enumerate(childrenCreators)]
    ...
    _relativeOverridableTypes = {
        kFloat, kDouble, kInt, kByte, kColor, kTime, kAngle, kDistance
    }
    _absoluteOverridableTypes = _relativeOverridableTypes | {
        kBool, kEnum, kString, kFilename
    }
    _connectionOverridableTypes = {
        kMessage, kObject
    }
    # 1 = (absolute bit flag), 2 = (relative bit flag) and 4 = (connection bit flag)
    # see TrenderSetup.h for details
    _availableOverrides = (lambda abslt, rel, con, types : {
        t:(1*(t in abslt) + 2*(t in rel)+ 4*(t in con)) for t in types
    })(_absoluteOverridableTypes, _relativeOverridableTypes, _connectionOverridableTypes, xrange(1,kLast))
    ...
    _typesToOverrideTypes = {
        ...
        kFilename : [typeIDs.absOverride, typeIDs.absOverride],
    ...
    _typesToApplyOverrideTypes = {
        ...
        kFilename : [typeIDs.applyAbsStringOverride, typeIDs.applyAbsStringOverride],
    ...
    _typesToLocalizedStrings = {
        ...
        kFilename : maya.stringTable['y_plug.kFilename' ],
    ...
    def availableOverrides(self):
        isVector = Plug._isVector(self.type)
        if not isVector and isinstance(self.type, list):
            return 0 # unsupported compound
        return Plug._availableOverrides.get(self.type[0] if isVector else self.type, 0)
    
    def acceptsOverrideType(self, typeId):
        bit = { typeIDs.absOverride.id() : 1, 
                typeIDs.relOverride.id() : 2,
                typeIDs.connectionOverride.id() : 4 }.get(typeId.id(),0)
        return bit != 0 and self.availableOverrides() & bit != 0
        ...
        if self._type is None:
            ...
            if attr.hasFn(OpenMaya.MFn.kMessageAttribute):
                ...
                if dataType == OpenMaya.MFnData.kString:
                    self._type = Plug.kFilename if OpenMaya.MFnAttribute(attr).usedAsFilename else Plug.kString
                else:
    ...
    def setAttributeLimits(self, limits):
        if self.hasLimits:
            fnclass, _ = Plug._creator(self.type)
