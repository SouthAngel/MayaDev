...
def _isRenderSetup(encodedData):
    return isinstance(encodedData, dict)
...
def getObjectNotes(encodedData):
    ...
    if _isRenderSetup(encodedData):
        ...
        if jsonTranslatorGlobals.SCENE_SETTINGS_ATTRIBUTE_NAME in keys:
...
def decodeObjectArray(list, policy):
    ...
    for d in list:
        ...
        if not objInst:
            OpenMaya.MGlobal.displayError(jsonTranslatorGlobals.kTypeNodeCreationFailed % nodeTypeName)
            continue
