...
class DefaultNodeExporter(rendererCallbacks.BasicNodeExporter):
    ...
    def __init__(self):
        super(DefaultNodeExporter, self).__init__()
...
def getAllRenderSettingsNodesInScene():
    ...
    renderSettingsNodeNames = getDefaultNodes()
    for name in renderSettingsNodeNames:
        yield commonUtils.nameToNode(name)

def sceneHasRenderSettingsApplyOverrides():
    return utils.sceneHasApplyOverridesOnAGivenNodeType(getAllRenderSettingsNodesInScene)

def encodeMasterValues():
    ''' Encode the master values of the attributes related to the Render
        Settings of a specific renderer '''
    with utils.EncodeMasterValuesCtxMgr(getAllRenderSettingsNodesInScene):
        return encode()
