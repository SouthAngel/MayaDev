...
def attributeToPlug(f):
    ...
    def attributeToPlugDecorator(*args, **kwargs):
        if '.' not in args[1]:
            args = list(args)
            nodeName = args[0].parent().getSelector().templateNodeName()
            if nodeName is None:
                raise commonUtils.ReportableException(maya.stringTable['y_override.kCouldntFigurePlugFromAttr' ] % (args[1], args[1]))
            args[1] = nodeName+"."+args[1]
        return f(*args, **kwargs)
    return attributeToPlugDecorator
    ...
    def setSource(self, source):
        with undo.CtxMgr("Set source '%s' for plug '%s'" % (source, self.name())):
            if source is None or cmds.objExists(source):
                ...
                OpenMaya.MGlobal.displayWarning("Missing source dependency '%s' for plug '%s'. '%s' does not exist." % (source, self.name(), source))
    ...
    def decode(self, dict):
        ...
        with undo.CtxMgr("Decode plug '%s'" % self.name()):
            with undo.NotifyCtxMgr("Decode plug '%s'" % self.name(), self._attrChanged):
                ...
                plug.Plug.createAttribute(self.handle.object(), self.longName, self.shortName, dict[self.longName])
            # set source if any
...
class Override(childNode.TreeOrderedItem, childNode.ChildNode):
    ...
    def setLocalRender(self, value):
        if value != self.isLocalRender():
            with undo.NotifyDecorateMgr("Set Local Render", context.beforeEnabled(self), context.afterEnabled(self)):
    ...
    def setSelfEnabled(self, value):
        if value != self.isSelfEnabled():
            with undo.NotifyCtxMgr("Set Override Enabled", self.enabledChanged):
    ...
    def hasApplyOverrides(self):
        return bool(utils.plugDst(self._getEnabledPlug()))
...
class ValueOverride(Override):
    ...
    def _isApplicableTo(self, plg):
        ...
        # override is applicable to plug if type compatible with override
        if not plg.accepts(self._getCompatibilityCheckPlug()):
            return False
        
        assert(plg.isConnectable)
        return True
    ...
    def _applyOne(self, plugToOverride, nextOvr=None):
    ...
    def applyInsertOne(self, node, attr, nextOvr=None):
        self._applyOne(commonUtils.findPlug(node, attr), nextOvr)
    ...
    def reapply(self, overridden):
        ...
        for (node, attrName, nextOv) in overridden:
            self._applyOne(commonUtils.findPlug(node, attrName), nextOv)
    ...
    def update(self):
        pass # nothing to do. DG dataflow pulls if needed.
    ...
    def acceptsAttr(self, plugName):
        plg = plug.findPlug(plugName)
        if plg is None:
            raise commonUtils.ReportableException(maya.stringTable['y_override.kCouldntFindPlug' ] % plugName)
        return plg.acceptsOverrideType(self.kTypeId)
...
class AbsOverride(LeafClass, ValueOverride):
    ...
    def compute(self, plg, dataBlock):
        ...
        if self._valuePlugHandle.isFinalized() and plg == self._valuePlugHandle.getPlug().plug:
            dataBlock.inputValue(plg)

        return Override.compute(self, plg, dataBlock)
...
class UniqueOverride(object):
    ...
    def addTargetAttribute(cls):
        strAttrFn = OpenMaya.MFnTypedAttribute()
        cls.kTargetNodeName = strAttrFn.create("targetNodeName", "tgName", 
...
class AbsUniqueOverride(UniqueOverride, AbsOverride):
    ...
    @staticmethod
    def initializer():
        AbsUniqueOverride.inheritAttributesFrom(AbsOverride.kTypeName)
        AbsUniqueOverride.addTargetAttribute()
...
class RelUniqueOverride(UniqueOverride, RelOverride):
    ...
    @staticmethod
    def initializer():
        RelUniqueOverride.inheritAttributesFrom(RelOverride.kTypeName)
        RelUniqueOverride.addTargetAttribute()
