...
class DragAndDropBehavior(OpenMayaUI.MPxDragAndDropBehavior):
    ...
    def connect(sourcePlug, destinationPlug):
        ...
        try:
            ...
            DragAndDropBehavior.raiseError(DragAndDropBehavior.kErrorMsg_IncompatibleTypes + " (%s -> %s)" % (sourcePlug.name(), destinationPlug.name()))
    ...
    def connectAttrToAttr(self, sourcePlug, destinationPlug, force):
        ...
        if fnDestinationNode.typeId == ConnectionOverride.kTypeId:
            ...
            if destinationPlug.partialName(useLongNames=True) == ConnectionOverride.kAttrValueLong:
                fnDestinationNode.userNode().setSource(sourcePlug.name())
        ...
        # Handle shader overrides node
            ...
            if destinationPlug.partialName(useLongNames=True) == ConnectionOverride.kAttrValueLong:
                ...
                if DragAndDropBehavior.isMatchingClass(sourceNode, "shader/surface"):
                    ...
                    DragAndDropBehavior.raiseError(DragAndDropBehavior.kErrorMsg_NoSurfaceShader + " (%s)" % fnSourceNode.name())
        ...
        # Handle material overrides node
            ...
            if destinationPlug.partialName(useLongNames=True) == ConnectionOverride.kAttrValueLong:
                ...
                if sgNode:
                    fnDestinationNode.userNode().setMaterial(
                        OpenMaya.MFnDependencyNode(sgNode).name())
                else:
                    DragAndDropBehavior.raiseError(DragAndDropBehavior.kErrorMsg_NoShadingGroupFound + " (%s)" % fnSourceNode.name())
        ...
        # Handle shader overrides node
            ...
            if destinationPlug.partialName(useLongNames=True) == ConnectionOverride.kAttrValueLong:
                ...
                if surfaceShaderNode:
                    fnDestinationNode.userNode().setShader(
                        OpenMaya.MFnDependencyNode(surfaceShaderNode).name())
                else:
                    DragAndDropBehavior.raiseError(DragAndDropBehavior.kErrorMsg_NoSurfaceShaderFound + " (%s)" % fnSourceNode.name())
