...
class ApplyOverride(OpenMaya.MPxNode):
    ...
    def setNotOnMainScene(self):
        ...
        # In certain practical cases, trying to undo the creation of that
        # attribute makes Maya crash. (MAYA-90352)
        # It has been decided that no objects associated to Render Setup should
        # be kept in the undo stack to optimize performance. Instead, we simply
        # do the inverse procedure on undo. For instance, we do not keep the
        # apply overrides when we unapply a layer, we delete them and undoing
        # the unapply of the layer simply calls the function that applies a
        # layer in the first place, recreating the correspondy apply overrides
        # in the process. Concretely, this means that undoing the creation of
        # an override will not keep any MObject associated to the apply
        # overrides, and thus they are deleted. Undoing the addition of a
        # dynamic attribute such as the current one would try to operate on a
        # deleted apply override, which leads to a crash since it is not
        # possible to call functions on a now nonexistent node. In conclusion,
        # it is not necessary to delete the attribute on undo, because it will
        # be deleted when the corresponding apply override is deleted.
        plug.Plug.createAttribute(self.thisMObject(), self.kNotOnMainSceneLong, 
                                  self.kNotOnMainSceneShort, properties, plug.kNotUndoable)
...
class ApplyAbsFloatOverride(LeafClass, ApplyAbsOverride):
    ...
    def compute(self, plug, dataBlock):
        return ApplyAbsOverride._compute(self, plug, dataBlock,
...
class ApplyRelFloatOverride(LeafClass, ApplyRelOverride):
    ...
    def compute(self, plug, dataBlock):
        return ApplyRelOverride._compute(self, plug, dataBlock,
            OpenMaya.MDataHandle.asFloat, 
            OpenMaya.MDataHandle.setFloat, 
            OpenMaya.MDataHandle.asFloat)
