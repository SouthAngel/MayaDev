...
from maya.app.renderSetup.common.test.sceneUtils import createShader, assignMaterial, setColor, ColorConstants, testShapeColor
...
def createRenderSetupScene():
    ...
    sphereName = 'aSphere'
    polySphereName = 'polySphere1'
    attrName = 'radius'
    cmds.polySphere(name=sphereName)

    rs = renderSetup.instance()

    rl = rs.createRenderLayer('aLayer')

    c = rl.createCollection('aCollection')

    c.getSelector().setFilterType(selector.Filters.kGenerators)
    c.getSelector().setPattern('polySphere*')

    o = c.createOverride('anOverride', override.AbsOverride.kTypeId)
    ovrValue = 5.0
    o.finalize(polySphereName + '.' + attrName)
    o.setAttrValue(ovrValue)

    return rl, polySphereName, attrName, ovrValue
...
def _createSceneWithMaterialShaderOverrides(typeId, setSource):
    (transform, polySphere) = cmds.polySphere()
    assert polySphere == 'polySphere1'
    shape = 'pSphereShape1'

    # Create 3 surface shaders of different colors
    blackShaderName, blackSgName = createShader("surfaceShader")
    setColor(blackShaderName + ".outColor", ColorConstants.BLACK)
    redShaderName, redSgName = createShader("surfaceShader")
    setColor(redShaderName + ".outColor", ColorConstants.RED)
    blueShaderName, blueSgName = createShader("surfaceShader")
    setColor(blueShaderName + ".outColor", ColorConstants.BLUE)

    # Assign the black shader to the sphere
    assignMaterial(shape, blackSgName)
    testShapeColor(shape, ColorConstants.BLACK)

    # Create a simple Render Setup scene with a render layer that has
    # the sphere as its member
    rs = renderSetup.instance()
    rl = rs.createRenderLayer('')
    rs.switchToLayer(rl)
    col = rl.createCollection('')
    col.getSelector().staticSelection.set([transform])

    # Create a shader/material override and set it to the red shader
    redOvr = col.createOverride('', typeId)
    setSource(redOvr, redShaderName, redSgName)
    testShapeColor(shape, ColorConstants.RED)

    # Create another shader/material override set to the blue shader but
    # disable it
    blueOvr = col.createOverride('', typeId)
    setSource(blueOvr, blueShaderName, blueSgName)
    blueOvr.setSelfEnabled(False)
    testShapeColor(shape, ColorConstants.RED)

    return rl, shape

def createSceneWithShaderOverrides():
    return _createSceneWithMaterialShaderOverrides(connectionOverride.ShaderOverride.kTypeId,
                                                lambda ovr, shader, sg: ovr.setSource(shader + ".outColor"))

def createSceneWithMaterialOverrides():
    return _createSceneWithMaterialShaderOverrides(connectionOverride.MaterialOverride.kTypeId,
                                                lambda ovr, shader, sg: ovr.setMaterial(sg))
