...
class BaseDelegate(QStyledItemDelegate):
    ...
    BACKGROUND_RECT_LENGTH = utils.dpiScale(28)
    BACKGROUND_RECT_LEFT_OFFSET = utils.dpiScale(4)
    ...
    def _drawArrow(self, painter, rect, item):
        painter.save()
        if item.rowCount() != 0:
            left = rect.left() + self.COLOR_BAR_WIDTH + 1
            arrow = None
            if self.treeView().isExpanded(item.index()):
                painter.translate(left + self.EXPANDED_ARROW_OFFSET, rect.top())
                arrow = self.EXPANDED_ARROW
            else:
                painter.translate(left + self.COLLAPSED_ARROW_OFFSET, rect.top())
    ...
    def drawAction(self, painter, actionName, pixmap, left, top, highlightedColor, drawDisclosure, borderColor):
        if (pixmap != None):
            ...
            # MAYA 84884: Created a backround rectangle underneath the icon to extend the mouse coverage region 
            backgroundRect = QRect(left - self.BACKGROUND_RECT_LEFT_OFFSET, top - self.ACTION_BORDER, self.BACKGROUND_RECT_LENGTH, self.ACTION_WIDTH)
            ...
            if not backgroundRect.contains(p):
