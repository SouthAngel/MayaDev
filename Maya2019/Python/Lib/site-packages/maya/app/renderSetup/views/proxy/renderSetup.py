...
RENDER_SETTINGS_CHILD_COLLECTION_TYPE = QStandardItem.UserType + RENDER_SETTINGS_CHILD_COLLECTION_TYPE_IDX
...
DEFAULT_LAYER_COLOR         = QColor(93, 93, 93)
DEFAULT_COLLECTION_COLOR    = QColor(82, 82, 82)
DEFAULT_OVERRIDE_COLOR      = QColor(71, 71, 71)
DEFAULT_TEXT_COLOR          = QColor(238, 238, 238)
DISABLED_TEXT_COLOR         = QColor(165, 165, 165)
...
class DataModelListObserver(object):
    ...
    def listItemAdded(self, listItem):
        ...
        # 
        # AttributeError: 'PySide2.QtGui.QStandardItem' object has no attribute 'depth'
        #
...
class Template(object):
    ...
    def findAllTemplateFiles(self, templateDirectory):
        ...
        for root, _, files in os.walk(templateDirectory):
            for file in files:
...
class ModelProxyItem(Template, standardItem.StandardItem):
    ...
    @undo.chunk('Paste')
    def paste(self, objList, clipBoardData):
        with guard.StateGuardCtx(jsonTranslatorGlobals.copyPasteLabelColorEnabled, jsonTranslatorGlobals.enableCopyPasteLabelColor, True):
            if self._model.isAcceptableChild(clipBoardData):
...
def _createControlForAttribute(attr, attrLabel, connectable=True):
    ...
    # when creating a string control.
        ...
        ctrl = cmds.textFieldGrp(label=attrLabel, preventOverride=True, fileName=True)
...
class OverrideProxy(LabelColor, ModelProxyItem):
    ...
    def data(self, role):
        if role == Qt.DisplayRole:
            ...
            if self._model.isLocalRender():
                return QColor(67,79,70)
            elif not self._model.isEnabled():
                return self.getDefaultColor()
            else:
                return self.getLabelColor()
        elif role == Qt.ForegroundRole:
            return DEFAULT_TEXT_COLOR if self._model.isSelfEnabled() \
    ...
    def acceptsDrops(self, attribute):
        return self._model.acceptsAttr(attribute)
...
class ConnectionUniqueOverrideProxy(UniqueOverrideProxy, ConnectionOverrideProxy):
    def __init__(self, model):
        super(ConnectionUniqueOverrideProxy, self).__init__(model)
...
class ShaderOverrideProxy(ConnectionOverrideProxy):
    ...
    def createAttributeUI(self, attribute):
        ...
        overrideCtrl = cmds.attrNavigationControlGrp(label="Override Shader", attribute=overrideAttr,
            createNew="shaderOverrideNewNode " + overrideAttr, connectToExisting="shaderOverrideReplaceNode " + overrideAttr)
        return overrideCtrl
...
class MaterialOverrideProxy(ConnectionOverrideProxy):
    ...
    def createAttributeUI(self, attribute):
        ...
        overrideCtrl = cmds.attrNavigationControlGrp(label="Override Material", attribute=overrideAttr,
            createNew="materialOverrideNewNode " + overrideAttr, connectToExisting="materialOverrideReplaceNode " + overrideAttr)
        return overrideCtrl
...
class BaseCollectionProxy(DataModelListObserver, LabelColor, ModelProxyItem):
    ...
    def data(self, role):
        if role == Qt.TextAlignmentRole:
            ...
            return self.getDefaultColor() if not self._model.isEnabled() \
                else self.getLabelColor()
        elif role == Qt.ForegroundRole:
            return DEFAULT_TEXT_COLOR if self._model.isSelfEnabled() \
                else DISABLED_TEXT_COLOR
        elif role == renderSetupRoles.NODE_ENABLED:
...
class RenderSettingsCollectionProxy(BaseCollectionProxy):
    ...
    def createCollection(self, collectionName, nodeType):
        collection = None
        assert nodeType == collectionModel.RenderSettingsChildCollection.kTypeId
        collection = self._model.createCollection(collectionName)
        proxy = getProxy(collection)
        return proxy.index()

    def supportsAction(self, action, numIndexes):
        return action in ([CREATE_RENDER_SETTINGS_CHILD_COLLECTION_ACTION,
                           SET_ENABLED_ACTION,
    ...
    def isAcceptableTemplate(self, objList):
        ...
        typename = jsonTranslatorUtils.getTypeNameFromDictionary(objList[0])
        overrideTypenames = { o.kTypeName for o in overrideUtils.getAllOverrideClasses() } | { collectionModel.RenderSettingsChildCollection.kTypeName }
        return typename in overrideTypenames
...
class RenderSettingsChildCollectionProxy(CollectionProxy):
    ...
    def __init__(self, model):
        super(RenderSettingsChildCollectionProxy, self).__init__(model)
        self.typeName = mel.eval('interToUI(\"%s\")' % self._model.superTypeName())
        self.addActiveLayerObserver()

    def aboutToDelete(self):
        """Cleanup method to be called immediately before the object is deleted."""
        super(RenderSettingsChildCollectionProxy, self).aboutToDelete()
        self.removeActiveLayerObserver()
    ...
    def typeIdx(self):
        return RENDER_SETTINGS_CHILD_COLLECTION_TYPE_IDX
...
class AOVCollectionProxy(BaseCollectionProxy):
    ...
    def isAcceptableTemplate(self, objList):
        typename = jsonTranslatorUtils.getTypeNameFromDictionary(objList[0])
        return typename in [collectionModel.AOVChildCollection.kTypeName]
...
class AOVChildCollectionProxy(BaseCollectionProxy):
    ...
    def isAcceptableTemplate(self, objList):
        typename = jsonTranslatorUtils.getTypeNameFromDictionary(objList[0])
        return typename in [overrideModel.AbsOverride.kTypeName, overrideModel.RelOverride.kTypeName]
...
class LightsProxy(BaseCollectionProxy):
    ...
    def onDoubleClick(self, view):
        import maya.app.renderSetup.lightEditor.views.editorUI as lightEditorUI
        lightEditorUI.createLightEditorWindow()
...
class RenderLayerProxy(DataModelListObserver, LabelColor, ModelProxyItem):
    ...
    def isAcceptableTemplate(self, objList):
        ...
        return typename in [o.kTypeName for o in collectionModel.getAllCollectionClasses()] \
            ...
            and typename!=collectionModel.RenderSettingsChildCollection.kTypeName \
            and typename!=collectionModel.AOVChildCollection.kTypeName
    ...
    def importTemplate(self, objList):
        ...
        with guard.StateGuardCtx(jsonTranslatorGlobals.importLabelColorEnabled, jsonTranslatorGlobals.enableImportLabelColor, True):
            self._model._decodeChildren(objList, renderSetupModel.DECODE_AND_RENAME, None)
...
class RenderSetupProxy(DataModelListObserver, Template, QStandardItemModel):
    ...
    def copyForClipboard(self, proxies):
        ...
        with guard.StateGuardCtx(jsonTranslatorGlobals.copyPasteLabelColorEnabled, jsonTranslatorGlobals.enableCopyPasteLabelColor, True):
            ...
            for proxy in proxies:
                proxyData = proxy._model.copyForClipboard()
    ...
    def exportSelectedToFile(self, filePath, notes, proxies):
        ...
        with open(filePath, "w+") as file:
            if proxies is None:
                json.dump(rs.encode(notes, userPrefs.ExportRenderSettingsAOVs.isEnabled()), fp=file, indent=2, sort_keys=True)
            else:
                data = []
                for proxy in proxies:
