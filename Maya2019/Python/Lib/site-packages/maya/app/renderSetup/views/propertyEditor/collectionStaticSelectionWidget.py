...
from PySide2.QtGui import QAbstractTextDocumentLayout, QColor, QKeySequence, QPainter, QPalette, QPen, QTextDocument, QTextOption
...
class CollectionStaticSelectionWidget(QListWidget):
    ...
    def __init__(self, selector, parent=None):
        ...
        self.sortAlphabeticallyAction = QAction(maya.stringTable['y_collectionStaticSelectionWidget.kSortAlphabetically' ], self.contextMenu)
        self.sortAlphabeticallyAction.triggered.connect(partial(self.setSortingEnabled, True))
        self.sortChronologicallyAction = QAction(maya.stringTable['y_collectionStaticSelectionWidget.kSortChronologically' ], self.contextMenu)
        self.sortChronologicallyAction.triggered.connect(partial(self.setSortingEnabled, False))
        ...
        self.setSortingEnabled(maya.app.renderSetup.views.proxy.renderSetup.getProxy(self._selector.owner()).isSortingEnabled())

    def populate(self):
        self.clear()
        # All our items are identical in height, so we do not compute their sizes each
        # time we add one.
        # This setting produces a huge performance gain for large data sets.
        # A negative side-effect is you dont get a horizontal scroll bar.
    ...
    def setSortingEnabled(self, enabled):
        super(CollectionStaticSelectionWidget, self).setSortingEnabled(enabled)
        maya.app.renderSetup.views.proxy.renderSetup.getProxy(self._selector.owner()).setSortingEnabled(enabled)
        if enabled:
            self.sortItems(Qt.AscendingOrder)
        elif self.count() != 0:
            names = { item.text() for item in self.selectedItems() }
            self.populate()
            self.highlight(names) # to keep previous selection
    ...
    def rightClicked(self, point):
        ...
        sep = QAction("", self.contextMenu)
        sep.setSeparator(True)
        actions.append(self.sortChronologicallyAction if self.isSortingEnabled() else self.sortAlphabeticallyAction)
