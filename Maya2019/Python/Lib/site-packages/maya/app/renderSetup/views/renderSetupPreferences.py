...
def _exportNodesOfGivenType(encodeFunction, captionName, filePath=None):
    ...
    selectedFilePath = [filePath] if filePath is not None else (
        cmds.fileDialog2(caption=captionName,
                         fileFilter='*.'+prefs.getFileExtension(),
                         dialogStyle=2,
...
def savePreset(captionName, fp=None):
    _exportNodesOfGivenType(renderSettings.encode, captionName, filePath=fp)

def exportAOVs(captionName, fp=None):
    _exportNodesOfGivenType(aovs.encode, captionName, filePath=fp)
...
def setDefaultPreset():
    renderSetupModel.setDefaultPreset()
...
def deleteUserPreset(preset, warn=True):
    ...
    message =  kDeleteUserRenderSettingsMsg % preset
    delete =  kDelete
    cancel =  kCancel
    if warn:
        confirmResponse = cmds.confirmDialog(title=kDeleteUserRenderSettings, button=[delete, cancel], cancelButton=cancel, defaultButton=delete, message=message)
