...
kRenderableToolTip = maya.stringTable['y_renderSetupDelegate.kRenderableToolTip' ]
kNonRenderableToolTip = maya.stringTable['y_renderSetupDelegate.kNonRenderableToolTip' ]
kRenderableSelectionToolTip = maya.stringTable['y_renderSetupDelegate.kRenderableSelectionToolTip' ]
kNonRenderableSelectionToolTip = maya.stringTable['y_renderSetupDelegate.kNonRenderableSelectionToolTip' ]
...
class RenderSetupDelegate(baseDelegate.BaseDelegate):
    ...
    kTooltips = {renderSetup.SET_VISIBILITY_ACTION : maya.stringTable['y_renderSetupDelegate.kVisibilityToolTip' ],
                 renderSetup.SET_RENDERABLE_ACTION : kRenderableToolTip,
    ...
    def _drawColorBar(self, painter, rect, item):
        ...
        # Seperator between the color bar and the regular node 
        rect3.setRight(rect3.left() + self.COLOR_BAR_WIDTH + 1)
        painter.fillRect(rect3, QColor(36,36,36))
    ...
    def _drawFill(self, painter, rect, item, isHighlighted, highlightColor):
        ...
        if isHighlighted:
            rect2.setLeft(rect2.left() + self.HIGHLIGHTED_FILL_OFFSET)
            if not item.data(renderSetupRoles.NODE_ENABLED):
                painter.drawTiledPixmap(rect2, baseDelegate.BaseDelegate.DISABLED_HIGHLIGHT_IMAGE)
            else:
            ...
            if not item.data(renderSetupRoles.NODE_ENABLED):
                painter.drawTiledPixmap(rect2, baseDelegate.BaseDelegate.DISABLED_BACKGROUND_IMAGE)
    ...
    def _setToolTipForRenderableAction(self, item):
        """ The tool tip for the SET_RENDERABLE_ACTION is dynamic, meaning that
            it changes depending on the item's current state.
            (depends on its renderability and whether it is currently selected)
        """
        if item.isSelected():
            if item.data(renderSetupRoles.NODE_RENDERABLE):
                item.setToolTip(kNonRenderableSelectionToolTip)
            else:
                item.setToolTip(kRenderableSelectionToolTip)
        else:
            if item.data(renderSetupRoles.NODE_RENDERABLE):
                item.setToolTip(kNonRenderableToolTip)
            else:
                item.setToolTip(kRenderableToolTip)
