    ...
    if renderSetupWindow is not None:
        if propertyEditor is None:
            ...
            if restore == True:
                ...
                minSize = editorMain.PropertyEditor.MINIMUM_SIZE
                requiredControl = renderSetupWindow.objectName() + 'WorkspaceControl'
                propertyEditor.show(dockable=True, retain=False, area='right', width=size.width(), widthSizingProperty='preferred', minWidth=minSize.width(), height=size.height(), controls=requiredControl, uiScript='import maya.app.renderSetup.views.renderSetup as renderSetup\nrenderSetup.createPropertyEditorUI(restore=True)', closeCallback='import maya.app.renderSetup.views.renderSetup as renderSetup\nrenderSetup.propertyEditorClosed()')
        else:
            propertyEditor.show()
...
def createUI(restore=False):
    ...
    if restore == True:
        ...
        minSize = rsWindow.RenderSetupWindow.MINIMUM_SIZE
        renderSetupWindow.show(dockable=True, retain=False, width=size.width(), widthSizingProperty='preferred', minWidth=minSize.width(), height=size.height(), x=250, y=200, plugins='renderSetup.py', uiScript='import maya.app.renderSetup.views.renderSetup as renderSetup\nrenderSetup.createUI(restore=True)', closeCallback='import maya.app.renderSetup.views.renderSetup as renderSetup\nrenderSetup.renderSetupWindowClosed()')
