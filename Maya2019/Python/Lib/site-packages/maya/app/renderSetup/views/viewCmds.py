...
def _itemMatches(item, renderLayers, overrides, collections, renderSettingsChildCollections):
    ...
    return ((renderLayers and item.type() == renderSetupProxy.RENDER_LAYER_TYPE) or
            ...
            (renderSettingsChildCollections and item.type() == renderSetupProxy.RENDER_SETTINGS_CHILD_COLLECTION_TYPE) or
...
def getSelection(renderLayers=False, collections=False, renderSettingsChildCollections=False, overrides=False):
    ...
    if not renderLayers and not collections and not renderSettingsChildCollections and not overrides:
        ...
        results = [selectedIndex.data(Qt.EditRole) for selectedIndex in selectedIndexes \
                   if _itemMatches(selectedIndex.model().itemFromIndex(selectedIndex), renderLayers, overrides, collections, renderSettingsChildCollections)]

    return results

def getSelectedCollections():
    # add the collections from the selected layers to the set
    selection = getSelection(True, False, False, False)
    ...
    selection = getSelection(False, True, False, False)
...
class RenderSetupSelectCmd(OpenMaya.MPxCommand):
    ...
    def createSyntax():
        ...
        syntax.addFlag(RenderSetupSelectCmd.kRenderSettingsChildCollectionsFlag, RenderSetupSelectCmd.kRenderSettingsChildCollectionsFlagLong, OpenMaya.MSyntax.kNoArg)
    ...
    def doIt(self, args):
        ...
        renderSettingsChildCollections = argDb.isFlagSet(RenderSetupSelectCmd.kRenderSettingsChildCollectionsFlag)
        overrides = argDb.isFlagSet(RenderSetupSelectCmd.kOverridesFlag)
        if not argDb.isQuery and (renderLayers or collections or renderSettingsChildCollections or overrides):
            raise RuntimeError(kNotEditableFlags)

        if argDb.isQuery:
            results = getSelection(renderLayers, collections, renderSettingsChildCollections, overrides)
