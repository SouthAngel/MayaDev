...
def dialogFactory(type):
    def _createOutputDialog(type, title):
        connection = 'view'
        if cmds.colorManagementPrefs(query=True, outputTransformUseColorConversion=True, outputTarget=type):
            connection = 'input'
        return OutputTransformDialog(connectionType=connection, outputTarget=type, dialogTitle=title)
    ...
    if type == 'rendering space':
        ...
        return _createOutputDialog(type, maya.stringTable['y_customTransformUI.kColorMgtCustomOutput' ])
    elif type == 'playblast':
        return _createOutputDialog(type, maya.stringTable['y_customTransformUI.kColorMgtCustomPlayblastOutput' ])
    else:
...
class OutputTransformDialog(Dialog):
    ...
    def __init__(self, path='', name='', transformConnection='', connectionType='', outputTarget='', dialogTitle=''):
        super(OutputTransformDialog, self).__init__(path=path, name=name, transformConnection=transformConnection)
    ...
    def apply(self):
        cmds.colorManagementPrefs(edit=True, outputTransformName=self.name, outputTarget=self.outputTarget)
# ===========================================================================
