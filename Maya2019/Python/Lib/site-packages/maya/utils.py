...
def loadStringResourcesForFile( scriptPath, fullModulePath, resourceFileName ):
    ...
    if fullModulePath != '' :
        path = 'python'
        fullModulePath = fullModulePath.replace("maya.", '')
        subPaths = fullModulePath.split('.')
    ...
    if scriptPath != '':
        localizedPath = os.path.normpath(os.path.join( scriptPath, 'scripts', path, resourceFileName ))
        try:
...
def loadStringResourcesForModule( moduleName ):
    ...
    if cmds.about( uiLanguageIsLocalized=True ):
        ...
        if loadStringResourcesForFile(scriptPath, fullModulePath, resourceFileName):
            return
